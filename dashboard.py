...
Serves:
    /            the CURRENT dashboard (v6 — warm mid ground, dark trace well)
    ...
    /old-5       v5, retired 2026-09-05 (learning panel + capped exchanges, on white)
...
def draft_context(target_ts, target_from=None, target_text=None,
    ...
    built for the RESPONDER and refuted (docs/addressing.md): it made Cal answer messages
...
<main>
  ...
  <div class="card faq" id="faq"><h2>FAQ — what this is and how it works</h2>
    ...
    <details><summary>What has to be true before Cal replies?</summary><div class="a">
      ...
      why — open <b>trace</b> on any exchange to see the whole ladder and exactly where it stopped.
      <i>Two paths run past the allow-list gate on purpose</i> — the greeting mirror and the signal report — because neither one writes prose, so the ladder that actually answered is drawn alongside the one that stopped.</div></details>
    <details><summary>Why do some messages say "OFF-LIST"?</summary><div class="a">
      ...
      allow-list can trigger a <b>written</b> reply. The fixed-table answers &mdash; greeting, radio check, contact report &mdash; are open to anyone, so an OFF-LIST message is not always a silent one. <i>Whether silence is the right behaviour is under
    ...
    <details><summary>Who can Cal talk to, and can it be switched off?</summary><div class="a">
      Only a small allow-list of nodes can reach the part of Cal that <b>writes prose</b>, and <b>anyone</b> on the mesh
      can read what Cal says. Three paths deliberately answer anyone at all, because none of them generates prose: a bare greeting gets the greeting mirrored back from a fixed table, and a radio check or a <b>contact report</b> &mdash; &ldquo;Got you in Olathe&rdquo; &mdash; gets this radio&rsquo;s own measurement of that packet read back. A test is addressed to whoever can hear it, so that is who it is answered for — the channel is public. Three independent always-on services do the work
...
# v6 — the ground moves off white to a warm mid neutral (2026-09-05). The trace panel
# keeps its own dark scope; see the design note inside the template.
PAGE_V6 = r"""<!doctype html>
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="color-scheme" content="light">
...
/* ==================== v4: the trace is a dark instrument well ====================
   ...
   to sit that is not the same plane as the list they came from. It also makes the
   elevation model honest in both directions: a raised plane is lighter than its
   ground and a well is darker, so the hierarchy v3 built survives the inversion
   ...
   What this cost: surfaces picked to sit just BELOW a white page (tabs, the gauge
   track, .norep, .xc.unprompted, the tab badge) were lighter than the new ground and
   would have read as RAISED, inverting the model above. They moved down with it.
...
<main>
  ...
  <div class="card faq" id="faq"><h2>FAQ — what this is and how it works</h2>
    ...
    <details><summary>How does Cal know a message is meant for it?</summary><div class="a">
      ...
      "<b>cal</b>endar" and "physi<b>cal</b>" do <i>not</i> trigger it.</div></details>
    <details><summary>What has to be true before Cal replies?</summary><div class="a">
      Being named isn't enough. First, the <b>signal report</b> gets its own narrower checks: a range
      or signal test is answered with what the radio measured, and nothing else runs. Otherwise, in
      order, a message must pass every gate: it's <b>not Cal's own</b> ·
      it's <b>fresh</b> · the responder is <b>enabled</b> · the sender is on the <b>allow-list</b> ·
      it's <b>addressed</b> · it's <b>within rate limits</b>. A message that passes them all and that
      no word rule claims can then be handed by the <b>router</b> to a capability, which keeps every
      refusal of its own. Miss one and Cal stays quiet and records
      why — open <b>trace</b> on any exchange to see the whole ladder and exactly where it stopped.</div></details>
    <details><summary>Why do some messages say "OFF-LIST"?</summary><div class="a">
      Because Cal <b>heard them perfectly well</b> and chose not to answer. Reception and reply are two
      different things: every message on the channel is received and shown here, but only senders on the
      allow-list can get a written reply. Two fixed answers reach everyone else: a <b>signal or range
      test</b> gets the radio&rsquo;s measured numbers, and a <b>bare greeting</b> on the open channel
      gets a short hello back, once a day per node. <i>Whether silence to everything else is the right
      behaviour is under active review</i> — the argument against it is that on a shared channel, staying
      quiet to one person while answering another isn't neutral, it reads as a snub.
      <br><a href="https://github.com/deanssamclaw/cal-mesh/blob/main/docs/proposals/unknown-sender-tier.md" target="_blank" rel="noopener noreferrer">The proposal to fix it ↗</a></div></details>
    <details><summary>Why are the replies so short?</summary><div class="a">
      Airtime is <b>shared by every node in range</b>, and LoRa has very little of it. A long message
      is not just slow, it takes the channel away from everyone else — including traffic that might
      ...
      code, and it's why the answers read like radio traffic rather than chat.</div></details>
    <details><summary>Who can Cal talk to, and can it be switched off?</summary><div class="a">
      Right now only a small allow-list of nodes can get a written reply; anyone else gets only a
      signal report or a fixed greeting back, and <b>anyone</b> on the mesh can read what Cal
      says — the channel is public. Three independent always-on services do the work
      (radio, cognition, this dashboard), so one can restart without dropping the others, and a single
      kill switch silences all automatic replies instantly. Note that node IDs are <b>not
      authenticated</b> and can be spoofed, so the allow-list is a courtesy control, not a security
      boundary. The real controls are the kill switch and the fact that the model can't run tools.</div></details>
    ...
    <details><summary>How does Cal choose what to say?</summary><div class="a">
      ...
      fetches a verified fact from a known source and hands it over, and the model's only job is to put
      that fact into words. We call it <b>capability injection</b>, and it's why Cal can't invent a
      temperature — if the fetch fails, it says so instead of guessing. For a message no word rule
      claims, a <b>decision model</b> may first route it to one of those capabilities instead; it picks
      a service and writes nothing.</div></details>
    <details><summary>Where does the weather come from?</summary><div class="a">
      The US National Weather Service, and nothing else — one allow-listed source, fetched by the
      software, never by the model. Cal reads the <b>latest observation from the nearest weather
      station</b> to a fixed reference point, and refuses to answer at all if that reading is too old.
    ...
    <details><summary>Why isn't it a real map?</summary><div class="a">
      ...
      clear to everyone in range.</div></details>
    <details><summary>What about privacy and safety?</summary><div class="a">
      The channel is public by design. This page shows the traffic Cal&rsquo;s node hears and sends
      — the open channel, Cal&rsquo;s own channel, and in their own tab the <b>direct messages</b> to and
      from Cal&rsquo;s node, published on purpose as a test bench — plus Cal's own telemetry, never the
      operator's data. Incoming text is treated as hostile: anyone in radio
      range can transmit anything, so messages are sanitized before they go anywhere near the model,
      the model runs with <b>no tools and no private context loaded</b>, and the trace reports
      <i>that</i> something was redacted and how many times, never <i>what</i>. An adversarial review
      of this exact path caught a real location leak before it shipped.
  ...
  <div class="card" id="changelog"><h2>Changelog</h2>
    <div class="clog">
      <div class="ci"><span class="cd">2026-09-24</span><b>Jev is now a backup, and switched on.</b> The router asks the scorer in the house first. Only when that scorer cannot answer &mdash; hot, busy or down &mdash; is the same question sent to Jev, from TypeSafe AI, and only for a message on the public channel: a direct message or anything on Cal&rsquo;s own channel is never sent, whatever else is switched on. Every use is recorded, counted on the capabilities page, and named in that message&rsquo;s trace, including what the house scorer said when it failed. Nothing else changes: Jev picks a service or nothing, and the service it picks still keeps every refusal it had.</div>
      <div class="ci"><span class="cd">2026-09-24</span><b>The second opinion is switched on, on the scorer in the house.</b> The operator&rsquo;s call. &ldquo;Cal, hows the link holding up?&rdquo; is now answered with the radio&rsquo;s measured numbers instead of the model&rsquo;s guess. An end-to-end review followed, and each finding was run rather than argued. It fixed a leak of command-line error text onto this page; a gap where a question about <i>another</i> station could be answered with the sender&rsquo;s own numbers &mdash; a signal report reached through this path now needs <b>every word</b> to be link-question vocabulary; and several failure paths that failed quietly. It also corrected several of our own claims that did not hold. &ldquo;Calibration was the lever&rdquo; was wrong: the gain came from how the scorer weighs its options, and the uncalibrated model makes the same rescues. And &ldquo;matches Jev&rdquo; was measured on labels that lean toward Jev, so it is weaker evidence than it sounded.</div>
      <div class="ci"><span class="cd">2026-09-23</span><b>A second opinion that stays in the house.</b> The router was built to ask a cloud service, which means message text leaving the house. Over two days a local scorer was measured against it on the same messages: a frozen open model, Qwen3.5-4B, scored by SemIf on a laptop in the house. Where it counts &mdash; the few messages confident enough to act on &mdash; it matched the cloud one: the same fixes, and nothing broken. So the router can run with no message text leaving the house.</div>
      <div class="ci"><span class="cd">2026-09-21</span><b>Built, not switched on: a second opinion on what a message is about.</b> Every capability claims a message with word rules, and a message no rule claims goes to the model &mdash; the one part of Cal that can invent. &ldquo;Cal, hows the link holding up?&rdquo; matched no rule and was once answered &ldquo;Link&rsquo;s solid&rdquo; with no number behind it. Now, <i>only</i> on that fallthrough, Cal can ask a decision model (Jev, from TypeSafe AI) one question: which service should answer this? Jev writes nothing and computes nothing; it can only hand the message to weather, the capability list or a signal report, each of which keeps every refusal it had, and two extra questions in the same call stop it sending a past-tense weather question to today&rsquo;s reading or a question about another station to your own signal numbers. Measured on the 319 distinct messages in the log, three times each: among messages addressed to Cal on the open channel it fixes <b>2 of 25</b> and breaks none &mdash; small on purpose. A wider version that also answered greetings and chatter not addressed to Cal scored far higher and was <b>not built</b>; that is a decision about airtime and privacy, not accuracy. An independent review found two serious problems in the first build and seven more, including this page still saying &ldquo;no model ran&rdquo; for a reply a model had routed. All fixed before anything was switched on. It is <b>off</b> (switched on 09-24, see above): switching it on sends message text to a third party, and that is the operator&rsquo;s call. Private messages stay home unless a second, separate switch says otherwise.</div>
...
<footer>cal-mesh dashboard v6 · auto-refresh 3s · read-only · previous version: <a class="faqlink" id="oldlink2" href="old-5">old-5</a></footer>
<script>
const $=s=>document.querySelector(s);
const DIR=(function(){let p=location.pathname.replace(/\/(v2|v3|v4|old-\d+)\/?$/,'/');
 return p.endsWith('/')?p:p+'/';})();
...
#
# To promote the next version: build PAGE_V7, move PAGE_V6 into RETIRED_PAGES as "old-6",
# point CURRENT_PAGE at PAGE_V7, and drop the stale entry from LEGACY_ALIASES.
CURRENT_PAGE = PAGE_V6
RETIRED_PAGES = {
    ...
    "old-5": PAGE_V5,      # v5 — learning panel + capped exchanges, on white. Retired 2026-09-05.
}
