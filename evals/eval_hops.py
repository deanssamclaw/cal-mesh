...
Run:  python3 evals/eval_hops.py                (exit 0 = pass)
      python3 evals/eval_hops.py --self-test    also proves the checks can FAIL
"""
...
if "--self-test" in sys.argv:
    ...
    for name, (target, old, new) in MUTANTS.items():
        ...
        # bridge.py holds no templates, so only the page file needs scoping
        mutated = (_mutate_current(src_txt, old, new) if target == "dashboard.py"
                   else src_txt.replace(old, new, 1))
        open(os.path.join(md, target), "w").write(mutated)
        shutil.copy(os.path.abspath(__file__), os.path.join(md, "eval_hops.py"))
