...
Run:  python3 evals/eval_guards.py     (exit 0 = pass; mutations included)
"""
import ast
import os
import re
...
EVALS = os.path.dirname(os.path.abspath(__file__))   # the suites themselves
FAILS = []
...
for fn in ("eval_routing.py", "eval_acks.py", "eval_routes.py"):
    c = code_of(os.path.join(EVALS, fn))
...
_declared = [f for f in sorted(os.listdir(EVALS))
             if f.startswith("eval_") and f.endswith(".py") and "SKIP" in code_of(os.path.join(EVALS, f))]
ck("every skip is announced with the word SKIP so the runner can see it",
   all("SKIP" in code_of(os.path.join(EVALS, f)) for f in _declared), _declared[:3])
...
_rev = []
for fn in sorted(f for f in os.listdir(EVALS) if f.startswith("eval_") and f.endswith(".py")):
    c = code_of(os.path.join(EVALS, fn))
...
for f in sorted(os.listdir(EVALS)):
    if not (f.startswith("eval_") and f.endswith(".py")):
        continue
    m = re.search(r"^def ck\(\s*([A-Za-z_]\w*)", code_of(os.path.join(EVALS, f)), re.M)
